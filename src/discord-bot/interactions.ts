 ...
 */
export async function handleInteractionCreateEvent(interaction: Interaction) {
  try {
    // 1) Check read-only mode up front
    if (BOT_READONLY_MODE) {
      logger("Bot is in read-only mode: ignoring slash commands and interactions.");
      return;
    }
    ...
    if (err instanceof Error) {
      logger(`Error in handleInteractionCreateEvent: ${err.message}`);
      console.error(err);
    } else {
...
async function handleButtonInteraction(interaction: ButtonInteraction) {
  ...
  await recordVote(interaction, nomineeId, roleName);
}
