    ...
    if (sqliteDb) {
      ...
      if (!existingGuild) {
        const oldGuild = await sqliteDb.get<{
          ...
          guild_name: string;
          date_added: string;
        }>("SELECT guild_id, guild_name, date_added FROM guilds WHERE guild_id = ?", [guildId]);
        if (oldGuild) {
      ...
      >("SELECT member_id, username, discriminator, guild_id FROM members WHERE guild_id LIKE ?", [`%${guildId}%`]);
      ...
      >("SELECT role_id, role_name, guild_id FROM roles WHERE guild_id = ?", [guildId]);
      ...
      >("SELECT user_id, role_id, guild_id, role_assigned_at FROM user_roles WHERE guild_id = ?", [guildId]);
