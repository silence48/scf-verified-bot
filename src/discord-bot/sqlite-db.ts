...
export async function storeGuild(guildId: string, guildName: string): Promise<void> {
  ...
  await db.run("INSERT OR REPLACE INTO guilds (guild_id, guild_name) VALUES (?, ?)", guildId, guildName);
}

/** Insert or update a role. */
export async function upsertRole(roleId: string, roleName: string, guildId: string): Promise<void> {
  const db = await getDatabase();
  await db.run(
    `
...
export async function getGuildMemberUsernames(guildId: string): Promise<string[]> {
  const db = await getDatabase();
  const rows = await db.all<{ username: string }[]>("SELECT username FROM members WHERE guild_id LIKE ?", [`%${guildId}%`]);
...
export async function insertVote(threadId: string, voterId: string): Promise<void> {
  const db = await getDatabase();
  await db.run("INSERT INTO votes (thread_id, voter_id, vote_timestamp) VALUES (?, ?, ?)", threadId, voterId, new Date().toISOString());
}

/** Return all voter_ids for a thread */
export async function getThreadVotes(threadId: string): Promise<string[]> {
  const db = await getDatabase();
  const rows = await db.all<{ voter_id: string }[]>("SELECT voter_id FROM votes WHERE thread_id = ?", [threadId]);
...
export async function getVotingThreadCreatedAt(threadId: string): Promise<Date | null> {
  const db = await getDatabase();
  const row = await db.get<{ created_at: string }>("SELECT created_at FROM voting_threads WHERE thread_id = ?", threadId);
...
export async function getThreadVoteCount(threadId: string): Promise<number | null> {
  const db = await getDatabase();
  const row = await db.get<{ vote_count: number }>("SELECT vote_count FROM voting_threads WHERE thread_id = ?", threadId);
...
export async function getExactGuildMemberUsernames(guildId: string): Promise<string[]> {
  const db = await getDatabase();
  const rows = await db.all<{ username: string }[]>("SELECT username FROM members WHERE guild_id = ?", [guildId]);
  return rows.map((r) => r.username);
}
