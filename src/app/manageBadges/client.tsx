...
export function BadgeManager({ initialBadges }: BadgeManagerProps) {
  ...
  async function handleBadgeUpdated(badgeId: string, updatedData: Partial<BadgeAsset>) {
    try {
      startTransition(async () => {
        ...
        setBadges((prevBadges) => prevBadges.map((badge) => (String(badge._id) === badgeId ? { ...badge, ...updatedData } : badge)));
      });
    } catch (error) {
  ...
  const filteredBadges = badges.filter((badge) => badge.code.toLowerCase().includes(filterText.toLowerCase()) || (badge.description_short || "").toLowerCase().includes(filterText.toLowerCase()));
      ...
      <div className="user-table-header">
        ...
        <div className="user-table-header-controls">
          <div className="user-table-search-wrapper">
            ...
            <input type="text" placeholder="Filter badges..." value={filterText} onChange={(e) => setFilterText(e.target.value)} className="user-table-search-input" />
          </div>
        </div>
      </div>
...
export function BadgeItem({ badge, onUpdate }: BadgeItemProps) {
  ...
  const editableFields = ["difficulty", "subDifficulty", "category_broad", "category_narrow", "description_short", "description_long", "current", "instructions", "type", "aliases"] as const;
        ...
        <div className="flex items-start gap-6">
          {badge.image && (
            <div className="shrink-0">
              <Image src={badge.image} alt={badge.code} width={120} height={120} className="rounded-md shadow-md" unoptimized />
            </div>
          )}

          <div className="flex-1">
            <h3 className="user-table-name mb-2">{badge.code}</h3>
            {!isEditing && badge.description_short && <p className="mb-3 text-gray-300">{badge.description_short}</p>}

            {!isEditing && (
              <button onClick={() => setIsEditing(true)} className="ui-button-base ui-button--default ui-button--default-size">
                Edit Badge
              </button>
            )}
          </div>
          ...
          <div className="user-table-detail-box mt-4">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {editableFields.map((key) => {
                const value = key === "aliases" && Array.isArray(formState[key]) ? (formState[key] as string[]).join(", ") : formState[key] !== undefined ? String(formState[key]) : "";

                return (
                  <div key={key} className="space-y-2">
                    <label className="user-table-label font-medium">{formatFieldName(key)}</label>
                    {key === "description_long" || key === "instructions" ? (
                      <textarea className="search-input w-full rounded-md border border-gray-700 bg-gray-800/50 shadow-sm" value={value} rows={4} onChange={(e) => handleChange(key, e.target.value)} />
                    ) : (
                      <input
                        className="search-input w-full rounded-md border border-gray-700 bg-gray-800/50 shadow-sm"
                        value={value}
                        onChange={(e) => handleChange(key, key === "current" ? Number.parseInt(e.target.value, 10) || 0 : e.target.value)}
            ...
            <div className="mt-6 flex justify-end space-x-3">
              ...
              <button onClick={handleSave} disabled={isPending} className="ui-button-base ui-button--default ui-button--default-size">
          ...
          <div className="mt-4">
            <div className="grid grid-cols-1 gap-x-6 gap-y-3 md:grid-cols-2">
              {[...readOnlyFields, ...editableFields]
                ...
                .map((key) => (
                  <div key={key} className="flex flex-col rounded-md border border-gray-700/50 bg-gray-800/30 p-2 shadow-sm">
                    <span className="user-table-label font-medium text-gray-400">{formatFieldName(key)}</span>
                    <span className="text-sm text-gray-300">{renderFieldValue(key as BadgeKey, badge[key as BadgeKey])}</span>
                  </div>
                ))}
            </div>
          </div>
