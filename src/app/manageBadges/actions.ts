...
const EDITABLE_FIELDS = ["difficulty", "subDifficulty", "category_broad", "category_narrow", "description_short", "description_long", "current", "instructions", "type", "aliases"] as const;
...
export async function updateBadge(badgeId: string, updatedData: Partial<BadgeAsset>): Promise<void> {
  ...
  const filteredData: Partial<EditableBadgeFields> = Object.fromEntries(
    Object.entries(updatedData).filter(([key]) => EDITABLE_FIELDS.includes(key as (typeof EDITABLE_FIELDS)[number])),
  ) as Partial<EditableBadgeFields>;

  await db.collection<BadgeAsset>("badges").updateOne({ _id: new ObjectId(badgeId) }, { $set: filteredData }, { upsert: true });
}
