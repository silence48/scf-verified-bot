...
export interface BaseBadgeHolder {
  _id?: ObjectId;
  owner: string;
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        console.warn(`🚨 Bad Request (404): ${url}`);
        return false;
      }
      ...
      if (response.status === 400) {
        console.warn(`🚨 Bad Request (400): ${url}`);
        return null; // Returning null so fetchAndUpdateAssetHolders can handle it
      }
  ...
  for (const record of records) {
    try {
      ...
      if (!isPaymentProcessed) {
        ...
        const claimedBalances = await processTransactionMeta(txMeta);
