...
export async function fetchAndUpdateAssetHolders(db: Db, asset: SqAsset) {
    ...
    let nextUrl = assetData.lastMarkUrlHolders
    ...
    try {
        while (nextUrl) {
            ...
            await badgesCollection.updateOne(
                { _id: badgeId },
                { $set: { lastMarkUrlHolders: String(data._links.self.href) } }
            );
        ...
        await holdersForBadgesCollection.updateOne(
            { badgeId },
            { $addToSet: { holders: { $each: Array.from(holdersSet) } } },
            { upsert: true }
        );
        ...
        await badgesCollection.updateOne(
            { _id: badgeId },
            { $set: { lastUpdatedHolders: new Date() } }
        );
  ...
  export async function ExistingInBadgeHolders(db: Db, assets: SqAsset[]): Promise<BadgeHolder[]> {
    const badges = await db.collection<Badge>("badges").find({
      $or: assets.map(a => ({ code: a.code, issuer: a.issuer }))
    }).toArray();
  
    const badgeIds = badges.map(badge => badge._id);
  
    return await db.collection<BadgeHolder>("BadgeHolders").find({
  ...
  export async function refreshAllAssetHolders(db: Db, assets?: SqAsset[]): Promise<void> {
    ...
    if (!assets || assets.length === 0) {
      assets = await badgesCollection.find().toArray().then(badges => 
