...
export async function parseTomlFiles(): Promise<boolean> {
  try {
    ...
    const questUrls = ["https://quest.stellar.org/.well-known/stellar.toml", "https://fastcheapandoutofcontrol.com/.well-known/stellar.toml"];
    ...
    for (const url of questUrls) {
      try {
        ...
        const existingBadges = await badgesCollection
          .find<BadgeAsset>({
            $or: currencyQueries,
          })
